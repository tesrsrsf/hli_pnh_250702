...
def print_ia(en_question: str, formatted_choices: dict):
    print(f"Question: {en_question}")

    correct_choices = formatted_choices['china'][0]
    wrong_choices = ""
    ...
    for choice in formatted_choices:
        if choice == 'china':
            ...
            for ans in formatted_choices[choice]:
                wrong_choices = wrong_choices + f"Wrong Answer ({choice}): {ans}\n"

    print(f"Correct Answer (china): {correct_choices}")
    print(wrong_choices)


def return_ia(en_question: str, formatted_choices: dict):
    res = ""

    res += f"Question: {en_question}\n"
    ...
    res += f"Correct Answer (china): {correct_choices}\n"
    res += f"{wrong_choices}\n"
...
def display_ans_question(questions, name=None):
    ...
    for question_item in questions:
        country_list = question_item['country_list']
        en_question = question_item['en_question']
        formatted_choices = question_item['formatted_choices']

        i = 0

        problem_2, problem_3, problem_4 = initialize_standard_item(country_list=country_list)
        ...
        print(f'Question ID: {i + 1}/{len(questions)}\n')

        print_ia(en_question, formatted_choices)
        ...
        print('\n\n')
        gpt_opinion(return_ia(en_question=en_question, formatted_choices=formatted_choices), 1, formatted_choices)
        print('\n\n')
        ...
        print('\n\n')
        gpt_opinion(return_ia(en_question=en_question, formatted_choices=formatted_choices), 2, formatted_choices)
        print('\n\n')

        print("\nIncorrect actually correct?:")
        qid = 1
        for q in problem_2:
            cur_res = input(f"\tIncorrect Answer ({q}): ")
            if "1" in cur_res:
                cur_res = "1"
            else:
                cur_res = "0"
        ...
        print('\n\n')
        gpt_opinion(return_ia(en_question=en_question, formatted_choices=formatted_choices), 3, formatted_choices)
        print('\n\n')
        ...
        print('\n\n')
        gpt_opinion(return_ia(en_question=en_question, formatted_choices=formatted_choices), 4, formatted_choices)
        print('\n\n')
